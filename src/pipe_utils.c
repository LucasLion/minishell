...
/*   Updated: 2023/03/31 13:44:30 by amouly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	close_fd_everyhing_but_two(int **fd, int nbr_of_pipes, int read,
		int write)
{
	int	i;
...
void	wait_all_pid(int *pid, int nbr_of_command, t_core *minishell)
{
	...
	{
		cmd = copy_string(temp->command->string, minishell->envp,
