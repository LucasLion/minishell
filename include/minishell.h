...
/*   Updated: 2023/03/31 17:33:34 by amouly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						exec_builtin(char *builtin, char **argv,
							t_core *minishell);
...
int						lstadd_back_list_command(t_command **list,
							t_command *new);
...
int						fill_list_string_append_or_heredoc(char *str,
							t_string **list);
...
void					free_export(char *val_arg, char *var_arg, char *var_env,
							char **envp);

/* -------------- PARSE.c -------------- */

void					handle_chevrons(char **tab, int index, t_command *new);
int						find_command_until_pipe(char **tab, int *i,
...
void					close_fd_everyhing_but_one(int **fd, int nbr_of_pipes,
							int a, int b);
void					close_fd_everyhing_but_two(int **fd, int nbr_of_pipes,
							int read, int write);
...
void					child_first_pipe(t_pipe *pipe_info, int **fd,
							char **envp);
void					child_last_pipe(t_pipe *pipe_info, int **fd,
							char **envp);
void					child_middle_pipe(t_pipe *pipe_info, int **fd,
							char **envp);
int						child_process(t_pipe *pipe_info, int **fd, char **envp);
int						managing_pipe(t_core *minishell, t_pipe *pipe_info,
							int **fd);
